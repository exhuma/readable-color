...
const colors: TColor[] = [
  { hex: "#123456", rgb: [18, 51, 86], hsl: [210, 65.000, 20.400] },
...
function test_conversions(color: TColor) {
  test(`RGB to HSL: ${color.rgb} -> ${color.hsl}`, () => {
    ...
    result.map((value, index) => expect(value/100).toBeCloseTo(color.hsl[index]/100, 1));
  });
  test(`HSL to RGB: ${color.hsl} -> ${color.rgb}`, () => {
    const result = hsl2rgb(...color.hsl);
    result.map((value, index) => expect(value / 100).toBeCloseTo(color.rgb[index] / 100, 1));
  });
